...
elif systemctl is-active --quiet routerbot 2>/dev/null; then
  ok "systemd: routerbot is active (legacy service name — reinstall with scripts/install-systemd.sh to rename)"
else
...
TUNNEL_URLS="${PARAGON_DIR:-${ROUTERBOT_DIR:-$(cd "$(dirname "$0")/.." && pwd)}}/data/tunnel-urls.json"
