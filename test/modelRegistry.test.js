...
test("buildModelRegistry marks antigravity automaticEligibility false, unrestricted risk", () => {
  const registry = buildModelRegistry(baseConfig(), {});
  const antigravity = registry.find((e) => e.provider === "antigravity");
  assert.equal(antigravity.automaticEligibility, false);
  assert.equal(antigravity.toolExecutionRisk, "unrestricted");
});

test("buildModelRegistry marks other providers automaticEligibility true, restricted risk", () => {
  const registry = buildModelRegistry(baseConfig(), {});
  const claude = registry.find((e) => e.provider === "claude" && e.model === "claude-opus-5");
  assert.equal(claude.automaticEligibility, true);
  assert.equal(claude.toolExecutionRisk, "restricted");
});
